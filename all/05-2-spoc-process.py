...
class scheduler:
    ...
    def move_to_ready(self, expected, pid=-1):
        ...
        if pid == -1:
            pid = self.curr_proc
        assert expected == STATE_RUNNING or expected == STATE_WAIT
        ...
        while self.get_num_active() > 0:
            ...
            for pid in range(len(self.proc_info)):
                if clock_tick in self.io_finish_times[pid]:
                    ...
                    self.move_to_ready(expected=STATE_WAIT, pid=pid)
            ...
            # and add an io completion in the future
                ...
                self.io_finish_times[self.curr_proc].append(
                    clock_tick + io_length)
                self.next_proc()

            # ENDCASE: check if currently running thing is out of instructions
            self.check_if_done()
            if self.proc_info[self.curr_proc] == STATE_READY:
                self.proc_info[self.curr_proc] = STATE_RUNNING
