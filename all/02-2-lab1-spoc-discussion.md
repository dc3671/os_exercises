...
>   * 对中断中断描述符表IDT进行初始化，为中断号和中断服务例程（包括段描述符和偏移地址）建立索引
>   * 对中断控制器进行初始化，完成相应配置
>   * 对时钟外设进行初始化
>   * 使能中断，调用一个特殊指令开启中断功能
...
>  1、先实现一个可显示字符串的bootloader，关键步骤是实现对时钟、串口和CGA的正常访问，以及从实模式切换到保护模式，使得段机制可以正常工作。需要注意的是模式切换必须正确。2、再实现一个可加载ELF格式文件的bootloader，关键步骤是实现对串口、并口的正常访问，从而可以加载硬盘扇区，把ucore代码从硬盘读取到内存指定位置后把控制权交给ucore操作系统。需要注意的事项是明确硬盘扇区指定文件的位置和大小。
...
>  * 使用linux指令strace
>  * 由于所有系统调用都需要访问中断向量表和系统调用表来确定具体的系统调用类型，可以修改操作系统代码，在进入中断向量表后（确定是系统调用）、进入系统调用表之前进行截断，跳转到一段附加代码，记录当前系统调用类型，之后再恢复原操作。
