...
```
 ...
 ```
- 先cons_init()初始化控制台，pmm_init()初始化物理内存，pic_init()初始化8259A中断控制，idt_init()初始化中断描述符表IDT。在clock_init()中初始化8253时钟，ticks置零，调用pic_enable()使能时钟。
...
lab1中完成了对哪些外设的访问？ (w2l2)
 ...
 ```
- 在cons_init()中调用了cga_init();serial_init();kbd_init();分别初始化了cga、串口、键盘的外设访问。
- 之后按照上题过程初始化时钟外设访问。
...
lab1中的cprintf函数最终通过哪些外设完成了对字符串的输出？ (w2l2)
 ```
  ...
  - 除上述两点外，进一步说明了CGA（3分）
 ```
- 通过understand查看调用关系，发现依次通过vcprintf、cputch调用了cons_putc()，用于打印一个字符在控制台上，调用了lpt_putc输出到并口，调用cga_putc输出到cga，serial_putc输出到串口。所以总共利用了这三个外设。
...
- 先实现系统的启动，仿照BIOS-MBR的实现机制，实现一个可以显示字符串的bootloader，分别先后实现初始寄存器内容；设备管理，封装串口、并口和CGA以完成字符和字符串的输出；实现实模式到保护模式的切换；实现设置栈；实现对字符串的显示，即在实模式转换到保护模式后可以在保护模式下通过PIO方式控制串口、并口和CGA等输出字符串并显示。
- 然后实现可读ELF格式文件的bootloader，实现基于可显示字符串的bootloader，首先可读硬盘，然后实现对ELF文件的分析和加载功能。同时注意保证bootloader的大小小于510 个字节，可以删减一些次要的功能。
- 然后增加一个可以显示字符的操作系统ucore，在bootloader将CPU转换为保护模式并加载ucore系统后，显示信息用于判断系统启动成功，这需要bootloader在完成两个任务之后，把PC指向ucore的代码，转移CPU控制权给ucore。
- 然后实现能够显示函数调用关系的ucore，分析出ucore在执行过程中的函数调用关系和函数传递的参数。
- 然后实现可以管理中断并处理中断方式I/O的ucore，通过中断机制完成设备中断请求处理，首先是初始化中断，涉及初始化中断控制器和中断描述符表和各种外设，然后是中断服务例程，在收到中断后进行处理。

> 


如何能获取一个系统调用的调用次数信息？如何可以获取所有系统调用的调用次数信息？请简要说明可能的思路。(spoc)
- 可以由操作系统内核在运行时使用一块空间来记录所有的系统调用的调用信息，在运行过程中根据程序行为维护这个表，并提供相应的API输出这些信息即可。  
