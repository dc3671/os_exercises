...
NTFS文件卷结构：
- 引导扇区BPB。大小为一个扇区
- 系统文件，NTFS一共有16个系统文件和8个保留文件
- File area数据区，六个用户的空间。
- MFT主文件表。
...
文件分配方式：
链式分配，以簇的形式分配，最小单位多扇区，N个扇区为一簇，簇大小一般为4KB，即8个扇区。使用逻辑簇号与虚拟簇号相结合的方式，逻辑簇号以卷为单位。映射时，通过簇大小与逻辑簇号的乘积得到卷地址。虚拟簇号是对用户文件而言的，指对该文件使用的簇号进行编号，如果知道了文件的开始地址，就可以用虚拟簇号映射到逻辑簇号得到地址。
